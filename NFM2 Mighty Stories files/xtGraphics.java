    ...
    implements Runnable
{
    ...
    public void cantgo(Control control)
    {
        ...
        if(aflk)
        {
            drawcs(160, "[ Track" + (unlocked + 1) + " Locked ]", 255, 128, 0, 3);
            aflk = false;
        } else
        {
            drawcs(160, "[ Track" + (unlocked + 1) + " Locked ]", 255, 0, 0, 3);
            aflk = true;
        }
        ...
        rd.setFont(new Font("SansSerif", 1, 11));
        ftm = rd.getFontMetrics();
        drawcs(396, "You can also use the mouse to navigate.", 82, 90, 0, 3);
        lockcnt--;
    ...
    public void loadingstage(int i)
    {
        ...
        rd.setFont(new Font("SansSerif", 1, 13));
        ftm = rd.getFontMetrics();
        drawcs(180, "Loading Track " + i + ", wait a second...", 0, 0, 0, 3);
        rd.drawImage(select, 273, 45, null);
        rd.drawImage(br, 0, 0, null);
        rd.setFont(new Font("SansSerif", 1, 11));
        ftm = rd.getFontMetrics();
    ...
    public void inst(Control control)
    {
        ...
        if(flipo == 3 || flipo == 5)
        {
            ...
            } else
            {
                ...
                rd.drawString("crash into them and make their damage bar full.", 197, 122);
                rd.setColor(new Color(0, 0, 0));
            }
        ...
        if(flipo == 15)
        {
            ...
            rd.drawString("I am Mighty Drifter by the way.", 197, 102);
            ...
            rd.drawString("=> To look behind while driving.", 202, 226);
            rd.drawImage(kv, 100, 250, null);
            rd.drawString("Change Camera", 142, 276);
    ...
    public void levelhigh(int i, int j, int k, int l, int i1)
    {
        ...
        {
            if(k == 0)
                drawcs(60, "You Killed 'em!", byte0, c, c1, 0);
            else
            if(k == 1)
                drawcs(60, "Close Finish!You won!", byte0, c, c1, 0);
            else
                drawcs(60, "Close Finish!You lose!", byte0, c, c1, 0);
        } else
        if(j == 229)
            drawcs(60, "Owned!", byte0, c, c1, 0);
        else
        if(i1 > 2)
            drawcs(60, "Insane Airs!", byte0, c, c1, 0);
        else
            drawcs(60, "Best Insane Air!", byte0, c, c1, 0);
        drawcs(380, "Press  [ Enter ]  to move on", 0, 0, 0, 0);
    }
    ...
    public void replyn()
    {
        if(aflk)
        {
            drawcs(30, "Highlight  > ", 0, 0, 0, 0);
            aflk = false;
        } else
        {
            drawcs(30, "Highlight  >>", 0, 128, 255, 0);
            aflk = true;
        }
    }
    ...
    public void nofocus()
    {
        ...
        rd.setFont(new Font("SansSerif", 1, 11));
        ftm = rd.getFontMetrics();
        drawcs(14, "Click screen with mouse to continue. NFM rocks!!!Do not exit from the game!!!", 100, 100, 100, 3);
        drawcs(395, "Click screen with mouse to continue. NFM rocks!!!Do not exit from the game!!!", 100, 100, 100, 3);
    }
    ...
    public void stat(Madness madness, CheckPoints checkpoints, Control control, boolean flag)
    {
        ...
        if(fase != -2)
        {
            ...
            if(checkpoints.wasted == 6)
            {
                ...
                if(aflk)
                {
                    drawcs(120, "You Won, all cars have been killed!", 0, 0, 0, 0);
                    aflk = false;
                } else
                {
                    drawcs(120, "You Won, all cars have been killed!", 0, 128, 255, 0);
                    aflk = true;
                }
                drawcs(350, "Press  [ Enter ]  to move on", 0, 0, 0, 0);
                ...
                do
                    if(checkpoints.clear[i] == checkpoints.nlaps * checkpoints.nsp && checkpoints.pos[i] == 0)
                    {
                        if(i == 0)
                        {
                            ...
                            if(aflk)
                            {
                                drawcs(120, "You passed the last checkpoint first!", 0, 0, 0, 0);
                                aflk = false;
                            } else
                            {
                                drawcs(120, "You passed the last checkpoint first!", 0, 128, 255, 0);
                                aflk = true;
                            }
                            winner = true;
                        } else
                        {
                            ...
                            if(aflk)
                            {
                                drawcs(120, "" + names[sc[i]] + " finished first, be faster next time!", 0, 0, 0, 0);
                                aflk = false;
                            } else
                            {
                                drawcs(120, "" + names[sc[i]] + " finished first, be faster next time!", 0, 128, 255, 0);
                                aflk = true;
                            }
                            winner = false;
                        }
            ...
            if(flag)
            {
                if(checkpoints.stage != 110 && arrace != control.arrace)
                {
                    ...
                    {
                        wasay = true;
                        say = " Arrow now pointing at  Victims  <";
                        tcnt = -5;
                    }
                    if(!arrace)
                    {
                        wasay = false;
                        say = " Arrow now pointing at  Checkpoints  <";
                        tcnt = -5;
                        cntan = 20;
                    }
                }
                    ...
                    if(!arrace && auscnt == 45 && madness.capcnt == 0)
                        if(madness.missedcp > 0)
                        {
                            if(madness.missedcp > 15 && madness.missedcp < 50)
                                if(flk)
                                    drawcs(70, "Checkpoint Missed! Get back to it!", 255, 0, 0, 0);
                                else
                                    drawcs(70, "Checkpoint Missed! Get back to it!", 255, 150, 0, 2);
                            madness.missedcp++;
                        ...
                        if(madness.mtouch && cntovn < 70)
                        {
                            ...
                            if(cntan > 40)
                            {
                                ...
                                if(flk)
                                {
                                    drawcs(70, "You 're going in the wrong way!", 255, 150, 0, 0);
                                    flk = false;
                                } else
                                {
                                    drawcs(70, "You 're going in the wrong way!", 255, 0, 0, 2);
                                    flk = true;
                                }
                            }
                        }
            ...
            if(!holdit)
            {
                    ...
                    if(tcnt == 30 && auscnt == 45 && madness.mtouch && madness.capcnt == 0)
                    {
                        if(looped != 2)
                        {
                            if(pwcnt < 70 || pwcnt < 160 && looped != 0)
                                if(pwflk)
                                {
                                    drawcs(110, "Power low, try to make an awesome stunt!", 0, 0, 200, 0);
                                    pwflk = false;
                                } else
                                {
                                    drawcs(110, "Power low, try to make an awesome stunt!", 255, 100, 0, 0);
                                    pwflk = true;
                                }
                        } else
                        if(pwcnt < 250)
                            if(pwflk)
                            {
                                ...
                                drawcs(120, "To learn how to make POWER!", 0, 0, 200, 0);
                                pwflk = false;
                            } else
                            {
                                drawcs(105, "> >  Press Enter for GAME INSTRUCTIONS!  < <", 255, 100, 0, 0);
                                drawcs(120, "To learn how to make POWER!", 255, 100, 0, 0);
                                pwflk = true;
                            }
                        pwcnt++;
                        if(pwcnt == 300)
                ...
                if(tflk)
                {
                    drawcs(110, "Are you kidding me?", 0, 0, 200, 0);
                    tflk = false;
                } else
                {
                    drawcs(110, "Are you kidding me?", 255, 100, 0, 0);
                    tflk = true;
                }
                if(madness.trcnt == 10)
                {
                    ...
                    if(auscnt < 45)
                    {
                        ...
                        if(!wasay)
                        {
                            tcnt = auscnt;
                            if(madness.power != 98F)
                                say = "Power Increased " + (int)((100F * madness.powerup) / 98F) + "%";
                            else
                                say = "Power Over MAX (+100%)";
                            if(skidup)
                                skidup = false;
                            else
                                skidup = true;
                ...
                do
                    if(dested[k] != checkpoints.dested[k])
                    {
                        ...
                        {
                            wasay = true;
                            say = "" + names[sc[k]] + " has been killed!";
                            tcnt = -15;
                        }
                        if(dested[k] == 2)
                        {
                            wasay = true;
                            say = "You owned " + names[sc[k]] + "!";
                            tcnt = -15;
                        }
                    }
                while(++k < 7);
    ...
    public void finish(CheckPoints checkpoints, ContO aconto[], Control control)
    {
        ...
        if(winner)
        {
            if(checkpoints.stage == unlocked)
            {
                if(checkpoints.stage != 17)
                {
                    ...
                    drawcs(80, "Track " + checkpoints.stage + " Conquered!", 170, 170, 170, 3);
                } else
                {
                ...
                if(checkpoints.stage != 17)
                {
                    ...
                    ftm = rd.getFontMetrics();
                    if(aflk)
                        drawcs(120 + pin, "Track " + (checkpoints.stage + 1) + " is now unlocked!", 176, 196, 0, 3);
                    else
                        drawcs(120 + pin, "Track " + (checkpoints.stage + 1) + " is now unlocked!", 247, 255, 165, 3);
                    if(byte0 != 0)
                    {
                        if(aflk)
                ...
                } else
                {
                    ...
                    ftm = rd.getFontMetrics();
                    if(aflk)
                        drawcs(90, "Woohoooo you completed the game!!!", 144, 167, 255, 3);
                    else
                        drawcs(90, "Woohoooo you completed the game!!!", 228, 240, 255, 3);
                    if(aflk)
                        drawcs(140, "Your Incredible!", 144, 167, 255, 3);
                    else
                        drawcs(142, "Your Incredible!", 228, 240, 255, 3);
                    if(aflk)
                        drawcs(190, "You're truly a Skilled GAMER!", 144, 167, 255, 3);
                    else
                        drawcs(190, "You're truly a Skilled GAMER!", 255, 100, 100, 3);
                    rd.setColor(new Color(0, 0, 0));
            ...
            } else
            {
                ...
                drawcs(137, "Track " + checkpoints.stage + " Conquered!", 170, 170, 170, 3);
        ...
        } else
        {
            ...
            drawcs(167, "Failed to Conquer Stage " + checkpoints.stage + "!", 170, 170, 170, 3);
    ...
    public void stageselect(CheckPoints checkpoints, Control control)
    {
        ...
        ftm = rd.getFontMetrics();
        if(checkpoints.stage != 17)
            drawcs(80, "Track " + checkpoints.stage + "  >", 255, 255, 255, 3);
        else
            drawcs(80, "Final Madness Stage  >", 255, 255, 255, 3);
        ...
        {
            asay = "Track " + checkpoints.stage + ":  " + checkpoints.name + " ";
            dudo = 150;
            m.trk = false;
            m.focus_point = 400;
            fase = 5;
    ...
    public void clicknow()
    {
        ...
        if(aflk)
        {
            drawcs(355, "Click here to Start the crazyness!!", 0, 0, 0, 3);
            aflk = false;
        } else
        {
            drawcs(355, "Click here to Start the crazyness!!", 0, 67, 200, 3);
            aflk = true;
        }
    }
    ...
    public void rad(int i)
    {
        ...
        if(radpx == 147)
        {
            rd.setFont(new Font("SansSerif", 1, 11));
            ftm = rd.getFontMetrics();
            drawcs(160 + (int)(5F * m.random()), "MightyDrifter100", 112, 120, 143, 3);
        }
        rd.setFont(new Font("SansSerif", 1, 11));
        ftm = rd.getFontMetrics();
        if(aflk)
        {
            drawcs(190, "Beware:You can get crazy while playing the game!!!", 112, 120, 143, 3);
            aflk = false;
        } else
        {
            drawcs(192, "Beware:You can get crazy while playing the game!!!", 150, 150, 150, 3);
            aflk = true;
        }
    ...
    public void cantreply()
    {
        ...
        drawcs(89, "You can't replay, so try again later.", 255, 255, 255, 1);
    }
    ...
    public void carselect(Control control, ContO aconto[], Madness madness)
    {
        ...
        if(flipo == 0)
        {
            ...
            if((sc[0] - 7) * 2 >= unlocked)
            {
                ...
                if(flatrstart == 6)
                {
                    drawcs(335, "[ Car Locked,Try Another One ]", 210, 210, 210, 3);
                    drawcs(355, "This car is ready to race when stage " + (sc[0] - 7) * 2 + " is conquered...", 181, 120, 40, 3);
                }
            } else
            {
                if(flatrstart == 6)
    ...
    public void loadingfailed(int i, Control control)
    {
        ...
        rd.setFont(new Font("SansSerif", 1, 13));
        ftm = rd.getFontMetrics();
        drawcs(140, "Can't Load Stage " + i, 200, 0, 0, 3);
        drawcs(170, "Your internet connection may have died...", 177, 177, 177, 3);
        drawcs(220, "Press Enter to die again.", 177, 177, 177, 3);
        rd.drawImage(contin[pcontin], 290, 325, null);
        rd.drawImage(br, 0, 0, null);
        rd.setFont(new Font("SansSerif", 1, 11));
        ftm = rd.getFontMetrics();
    ...
    public void hipnoload(int i, boolean flag)
    {
        ...
        if(byte0 == 0)
        {
            ...
            if(i == 1)
            {
                rd.drawString("Get out from the deep sea before the racers see you!", 197, 67);
                rd.drawString("Or try to waste them if you can!", 197, 87);
            }
            if(i == 2)
                rd.drawString("Find your way out before you become blood!!!", 197, 67);
            if(i == 3)
            {
                rd.drawString("You are on MASHEEN's teritory.Lucky,he isn't home.", 197, 67);
                rd.drawString("He sent a cop to protect his teritory from racing and wasting!!!", 197, 87);
                rd.drawString("You better run before he sees you!!!", 197, 127);
            }
            if(i == 4)
            {
            ...
            if(i == 7)
            {
                rd.drawString("Welcome to the realm of the heaven!!!", 197, 67);
                rd.drawString("You are stranded above the sky! The only way to escape is to ", 197, 107);
                rd.drawString("defeat the King! Good luck!", 197, 127);
            }
            if(i == 8)
            {
                rd.drawString("Watch out, EL KING is out to get you now in the sea!", 197, 67);
                rd.drawString("He seeks revenge and he will try to waste you!", 197, 87);
            ...
            if(i == 9)
                rd.drawString("Hint: Audi R8 Tuned is very fast, but also very weak!", 197, 67);
            if(i == 10)
            {
                rd.drawString("You have been challenged by Audi R8 Tuned in a race with a million", 197, 67);
                rd.drawString("dollar prize! But the Audi R8 Tuned is very fast!", 197, 87);
                rd.drawString("(Hint: You may need to do more forward loops here.  Also try keeping", 197, 127);
                rd.drawString("your power at maximum at all times. ).", 197, 147);
            }
            if(i == 12)
            {
                rd.drawString("Watch out!  Look out!  Take care!", 197, 67);
                rd.drawString("MASHEEN is hiding out in the snow, don't get wasted now!", 197, 87);
            }
            if(i == 13)
            {
                rd.drawString("Radical Stunter is on the field with his minions!", 197, 67);
                rd.drawString("Have fun wasting them with MASHEEN!", 197, 87);
            }
            if(i == 16)
            {
                rd.drawString("This is it!  This is the toughest stage in the game!", 197, 67);
                rd.drawString("The sandstorm has come and you can't get away from it!", 197, 107);
                rd.drawString("The only way to escape is to follow the road!!! But make sure ", 197, 127);
                rd.drawString("nobody tries to waste you!  Good luck and be fast!", 197, 147);
            }
        }
        ...
        if(!flag)
        {
            ...
            drawcs(350 + byte0, "Wait a second...", 0, 0, 0, 3);
        } else
        {
            drawcs(340 + byte0, "Loading complete!  Press Enter to begin...", 0, 0, 0, 3);
    ...
    int dested[];
    String names[] = {
        "Dirty Shark", "Dragrace Runner", "Crazy Minivan", "Purple Crab", "Tuned Mini Cooper", "GTR Revenge", "Black Oxide", "Chocolate Lemon Kat", "Drifter Z", "Ford Crown Victoria(Police Version)", 
        "Ridge Rider", "EL KILLER", "Audi R8 Tuned", "Dirty Bulldozer", "Radical Stunter", "Monster Truck"
    };
    int dmcnt;
    boolean dmflk;
    int pwcnt;
    ...
    String adj[][] = {
        {
            ...
            "Awesome", "Very Crazy", "Incredible"
        }, {
            "You are the super star!!!", "Your a sky dancer!!!!", "Who do you think you are?"
        }, {
