...
label0:
    ...
    void loadMiniMap(ContO[] co, int i)
    {
        ...
        {
            Arrays.sort(tpolys, list.get(A), list.get(A + 1), new Comparator<Integer[]>() {
                @Override
                public int compare(final Integer[] entry1, final Integer[] entry2) {
                    final Integer time1 = entry1[3];
                    final Integer time2 = entry2[3];
                    return time2.compareTo(time1);
                }
            });
        }
        ...
        if(fase != -2)
        {
            ...
            if(flag)
            {
                ...
                if(control.minion) {
                    ...
                    ///////g.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
                    g.setComposite(AlphaComposite.getInstance(3, .7F));
                    g.setColor(new Color(m.cgrnd[0], m.cgrnd[1], m.cgrnd[2]));
                    g.fillRect(0, 56, 200, 200);
                    g.setComposite(AlphaComposite.getInstance(3, 1));
                    g.setClip(new Rectangle2D.Double(0, 56, 200, 200));
                    g.rotate(checkpoints.omxz[0] * 0.017453292519943295D, 100, 156);
                    rate /= zoom;
                    g.drawImage(map, (int)((checkpoints.opx[0] - criticalB[1]) / -rate) + 100, (int)((checkpoints.opz[0] - criticalB[2]) / rate) + 156, (int)(map.getWidth() * zoom), (int)(map.getHeight() * zoom), null);
                    g.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
                    for (byte pN = 0; pN < 8; pN++)
                    {
                        if (pN == 0)
                            g.setColor(Color.WHITE);
                        else if(alocked != -1 && alocked == pN)
                            g.setColor(Color.YELLOW);
                        else
                            g.setColor(Color.BLACK);
                        if ((madness[pN].hitmag / madness[pN].maxmag[madness[pN].cn]) < 1)
                        {
                            pointX[0] = (checkpoints.opx[pN] - checkpoints.opx[0]) / rate + 100 + 4 * StrictMath.sin(Math.toRadians(-checkpoints.omxz[pN]));
                            pointX[1] = (checkpoints.opx[pN] - checkpoints.opx[0]) / rate + 100 + 2.828427124746190097 * StrictMath.sin(Math.toRadians(135 - checkpoints.omxz[pN]));
                            pointX[2] = (checkpoints.opx[pN] - checkpoints.opx[0]) / rate + 100 + 2.828427124746190097 * StrictMath.sin(Math.toRadians(-checkpoints.omxz[pN] - 135));
                            pointZ[0] = (checkpoints.opz[0] - checkpoints.opz[pN]) / rate + 156 - 4 * StrictMath.cos(Math.toRadians(-checkpoints.omxz[pN]));
                            pointZ[1] = (checkpoints.opz[0] - checkpoints.opz[pN]) / rate + 156 - 2.828427124746190097 * StrictMath.cos(Math.toRadians(135 - checkpoints.omxz[pN]));
                            pointZ[2] = (checkpoints.opz[0] - checkpoints.opz[pN]) / rate + 156 - 2.828427124746190097 * StrictMath.cos(Math.toRadians(-checkpoints.omxz[pN] - 135));
                    ...
                    g.setColor(Color.BLACK);
                    g.drawRect((int)((checkpoints.opx[0] - criticalB[1]) / -rate) + 100, (int)((checkpoints.opz[0] - criticalB[2]) / rate) + 156, (int)(map.getWidth() * zoom), (int)(map.getHeight() * zoom));
                    g.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_OFF);
                    rate *= zoom;
                    g.rotate(-checkpoints.omxz[0] * 0.017453292519943295D, 100, 156);
                    g.setClip(new Rectangle2D.Double(0, 36, 240, 240));
                    g.setColor(m.darksky || m.lightson? new Color(50, 204, 255):colorSnap(50, 204, 255));
