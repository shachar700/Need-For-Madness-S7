...
public class ContO
{
    ...
    public void sprk(float f, float f1, float f2, float f3, float f4, float f5, int i)
    {
        if(i != 1)
        {
            ...
            sry = (int)(f1 - (float)sprkat * m.cos(zy) * m.cos(xy));
            srz = (int)(f2 + (float)sprkat * m.cos(xz));
            sprk = 1;
        } else
        {
            sprk++;
    ...
    public void dsprk(Graphics2D graphics2d, boolean flag)
    {
        ...
        for(int j = 0; j < 100; j++)
        {
            ...
            if(rtg[j] == 1)
            {
                if(sprk < 5)
                {
                    rx[j] = (srx + 3) - (int)((double)m.random() * 6.7000000000000002D);
                    ry[j] = (sry + 3) - (int)((double)m.random() * 6.7000000000000002D);
                    rz[j] = (srz + 3) - (int)((double)m.random() * 6.7000000000000002D);
                } else
                {
                ...
                if(m.random() > m.random())
                {
                    if(m.random() > m.random())
                        f2 *= -1F;
                    vrx[j] = -((rcx + (float)k * (1.0F - rcx / (float)k) * f1 * f2) * f);
                }
                if(m.random() > m.random())
                {
                    if(m.random() > m.random())
                    ...
                    if(sprk == 5)
                        f2 = 1.0F;
                    vry[j] = -((rcy + (float)k * (1.0F - rcy / (float)k) * f1 * f2) * f);
                }
                if(m.random() > m.random())
                {
                    if(m.random() > m.random())
                        f2 *= -1F;
                    vrz[j] = -((rcz + (float)k * (1.0F - rcz / (float)k) * f1 * f2) * f);
                }
            }
            rx[j] += vrx[j];
            ry[j] += vry[j];
            rz[j] += vrz[j];
            int l = m.cx + (int)((float)(rx[j] - m.x - m.cx) * m.cos(m.xz) - (float)(rz[j] - m.z - m.cz) * m.sin(m.xz));
            int j1 = m.cz + (int)((float)(rx[j] - m.x - m.cx) * m.sin(m.xz) + (float)(rz[j] - m.z - m.cz) * m.cos(m.xz));
            int l1 = m.cy + (int)((float)(ry[j] - m.y - m.cy) * m.cos(m.zy) - (float)(j1 - m.cz) * m.sin(m.zy));
            j1 = m.cz + (int)((float)(ry[j] - m.y - m.cy) * m.sin(m.zy) + (float)(j1 - m.cz) * m.cos(m.zy));
            int i2 = m.cx + (int)(((float)(rx[j] - m.x - m.cx) + vrx[j]) * m.cos(m.xz) - ((float)(rz[j] - m.z - m.cz) + vrz[j]) * m.sin(m.xz));
            int j2 = m.cz + (int)(((float)(rx[j] - m.x - m.cx) + vrx[j]) * m.sin(m.xz) + ((float)(rz[j] - m.z - m.cz) + vrz[j]) * m.cos(m.xz));
            int k2 = m.cy + (int)(((float)(ry[j] - m.y - m.cy) + vry[j]) * m.cos(m.zy) - (float)(j2 - m.cz) * m.sin(m.zy));
            j2 = m.cz + (int)(((float)(ry[j] - m.y - m.cy) + vry[j]) * m.sin(m.zy) + (float)(j2 - m.cz) * m.cos(m.zy));
            int l2 = xs(l, j1);
            int i3 = ys(l1, j1);
            int j3 = xs(i2, j2);
            int k3 = ys(k2, j2);
    ...
    public ContO(ContO conto, int i, int j, int k, int l)
    {
        ...
        for(int i1 = 0; i1 < npl; i1++)
        {
            if(conto.p[i1].master != 0)
                conto.p[i1].n = 16;
            p[i1] = new Plane(m, t, conto.p[i1].ox, conto.p[i1].oz, conto.p[i1].oy, conto.p[i1].n, conto.p[i1].oc, conto.p[i1].glass, conto.p[i1].gr, conto.p[i1].fs, conto.p[i1].wx, conto.p[i1].wy, conto.p[i1].wz, conto.disline, conto.p[i1].bfase, conto.p[i1].road, conto.p[i1].light, conto.p[i1].dontdraw);
        }
