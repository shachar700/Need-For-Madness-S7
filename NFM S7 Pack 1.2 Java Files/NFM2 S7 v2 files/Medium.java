...
public class Medium {
    ...
    int rescnt;
    int dispolys = 0;
    int[][] ogpx = new int[14500][8];
    int[][] ogpz = new int[14500][8];
    int[] cgpx = new int[14500];
    float[] pcv;
    float lastprf;
    int[] twn;
    int[] cgpz = new int[14500];
    int sgpx = 0;
    int sgpz = 0;
    int nrw = 0;
    int[] stx;
    ...
    public void getfollow(ContO conto, int i, int j, boolean flag, Control control) {
        this.zy = 10;
        int k = 2 + Math.abs(this.bcxz) / 4;
        if (j != 0) {
            if (j == 1) {
        ...
        this.xz = -(i += this.bcxz);
        int l = conto.x - this.cx + (int)((float)(-(conto.z - 800 - conto.z)) * this.sin(i));
        int i1 = conto.z - this.cz + (int)((float)(conto.z - 800 - conto.z) * this.cos(i));
        int j1 = conto.y - 250 - this.cy;
        int k1 = 0;
        if (Math.abs(j1 - this.y) > this.fvect) {
            this.y = this.y < j1 ? (this.y += this.fvect) : (this.y -= this.fvect);
        } else {
            this.y = j1;
            ++k1;
        }
        if (Math.abs(l - this.x) > this.fvect) {
            this.x = this.x < l ? (this.x += this.fvect) : (this.x -= this.fvect);
        } else {
            this.x = l;
            ++k1;
        }
        if (Math.abs(i1 - this.z) > this.fvect) {
            this.z = this.z < i1 ? (this.z += this.fvect) : (this.z -= this.fvect);
        } else {
            this.z = i1;
            ++k1;
        }
        this.fvect = k1 == 3 ? 200 : (this.fvect += 2);
    }
